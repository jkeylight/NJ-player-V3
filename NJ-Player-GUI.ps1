...
try {
    ...
    Write-Error "Failed to load .NET assemblies: $_"
    exit 1
}
...
function Play-Video {
    ...
    $script:mpvProc = Start-Process -FilePath $MpvExe -ArgumentList $mpvArgs -PassThru
    $script:mpvWatchTimer = New-Object System.Windows.Forms.Timer
    $script:mpvWatchTimer.Interval = 1000
    $script:mpvWatchTimer.Add_Tick({
        if ($script:mpvProc -and $script:mpvProc.HasExited) {
            $script:mpvWatchTimer.Stop()
            $script:mpvWatchTimer.Dispose()
...
# STARTUP
# ============================================
$startFolder = $LibraryDir
if (-not (Test-Path $startFolder)) {
    try { New-Item -Path $startFolder -ItemType Directory -Force | Out-Null } catch {}
}
$script:currentFolder = $startFolder
try {
    Load-Videos -Folder $startFolder
} catch {
    $statusLabel.Text = "Ready - Use Browse to select a folder"
}
